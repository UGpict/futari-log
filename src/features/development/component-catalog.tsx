...
const migrations = [
  ...
  ["入力欄・テキストエリア", "home/plan-form・home/home-screen・memory/memory-screen・session/session-screen・session/session-details", "Field + 共通入力部品（未作成）", "Field はラベルのみ。入力本体の色・枠・focus を共通化してから移行。"],
...
export function ComponentCatalog({ inventory, literalColors, colorTokens }: CatalogData) {
  ...
  return <main className={styles.page}>
    ...
    <section id="examples" className={styles.section}><h2>実物プレビュー</h2><p>hover はマウス、focus は Tab キーで確認できます。</p><div className={styles.grid}>
      ...
      <Example name="Card / Field" note="Card は見出し付きセクション。Field はラベル枠のみ。使用状況は自動取得する台帳で確認。"><Card title="ふたりのメモ"><Field label="タイトル"><input className={styles.sampleInput} placeholder="のんびり過ごす一日" /></Field></Card><p>※ 入力本体の枠は一覧用。共通入力コンポーネントはまだありません。</p></Example>
