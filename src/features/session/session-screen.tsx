    ...
    <section className={styles.itinerary} aria-label="この日のスケジュール"><h2 className="sr-only">この日のスケジュール</h2>
      ...
      {(() => {
        ...
        const endLeg = last
          ...
          : null;
        return endLeg && !proposalReady ? <p className={styles.transition}><span />解散まで {legDurationLabel(endLeg)} · {travelModeLabel(endLeg.mode, endLeg.walkMinutesWithin?.value)}</p> : null;
      })()}
