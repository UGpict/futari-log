...
import { ArrowLeft, Check, Eye, EyeOff, LockKeyhole, LogOut, Mail } from "lucide-react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { useState, type FormEvent } from "react";

import { api } from "@/client/api";
import { forgetAuthEntry, rememberAuthEntry } from "@/client/auth-entry";
...
const viewCopy: Record<AuthView, { eyebrow: string; title: string; description: string }> = {
  ...
  verify: {
    eyebrow: "あと少しです",
    title: "メールを確認してください",
    description: "確認メール内のリンクを開いてください。",
  },
  "forgot-password": {
...
export function AuthFlowScreen({
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (view === "login") {
        await api<AuthSessionResponse>("/api/auth/login", {
          method: "POST",
      ...
      if (view === "signup") {
        await api<AuthSessionResponse>("/api/auth/signup", {
          method: "POST",
          body: JSON.stringify({ email, password }),
        });
        rememberAuthEntry("account");
    ...
    <main className={styles.page}>
      <section className={styles.screen} aria-labelledby="auth-title">
        <header className={styles.header}>
          <ButtonLink href={backHref} variant="ghost" size="compact">
            <ArrowLeft />
            戻る
          </ButtonLink>
        ...
        <div className={styles.intro}>
          <span className={styles.icon} aria-hidden="true">
            {view === "verify" ? <Mail /> : view === "logout" ? <LogOut /> : <LockKeyhole />}
          </span>
          <p>{copy.eyebrow}</p>
          <h1 id="auth-title">{copy.title}</h1>
          <span>{view === "verify" ? verifyDescription : copy.description}</span>
        </div>
        ...
        <form className={styles.form} onSubmit={(event) => void onSubmit(event)}>
            ...
            <label className={styles.field}>
              ...
              <span className={styles.inputWrap}>
                ...
                <IconButton
                  className={styles.eyeButton}
                  type="button"
                  label={showPasswordConfirm ? "パスワード（確認）を隠す" : "パスワード（確認）を表示"}
          ...
          {view === "login" && (
            <>
              ...
              <p className={styles.switch}>
                はじめての方は <Link href="/auth/signup">アカウントを作る</Link>
              </p>
            </>
          )}
          ...
          {view === "signup" && (
            <>
              <p className={styles.note}>8文字以上で入力してください</p>
              <Button fullWidth type="submit" disabled={busy}>
          ...
          {view === "forgot-password" && (
            <>
              <Button fullWidth type="submit" disabled={busy}>
                {busy ? "送信しています…" : "再設定メールを送る"}
              </Button>
              <ButtonLink fullWidth variant="secondary" href="/auth/login">
                ログインへ戻る
              </ButtonLink>
            </>
          )}
          ...
          {view === "verify" && (
            <div className={styles.simpleActions}>
              ...
              <Button fullWidth type="button" disabled={busy} onClick={() => void checkVerified()}>
                {busy ? "確認しています…" : "確認が完了したかチェック"}
              </Button>
              <Button fullWidth variant="secondary" type="button" disabled={busy} onClick={() => void resendVerification()}>
                確認メールを再送
              </Button>
              <ButtonLink fullWidth variant="ghost" href="/auth/login">
                別のアカウントを使う
              </ButtonLink>
            </div>
          )}
          ...
          {view === "logout" && (
            <div className={styles.simpleActions}>
              <Button fullWidth variant="danger" type="button" disabled={busy} onClick={() => void logout()}>
                <LogOut />
                {busy ? "ログアウトしています…" : "ログアウト"}
              </Button>
              <ButtonLink fullWidth variant="secondary" href="/">
                キャンセル
              </ButtonLink>
            </div>
          )}
