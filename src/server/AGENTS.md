...
- イベントカタログは `catalogEvents` / `catalogVenues` / `catalogIngestRuns`（sys/root に載せない）。検索は OrcaRouter の Gemini `googleSearch`（native generateContent）。一覧の PARTIAL と確定プラン適用は分ける。ENABLE_EVENT_CATALOG でプラン接続。選択イベントが使えないときは Places に黙って置き換えず `q_event_fallback`。
