...
このリポジトリの既定実行は **MOCK + Auth/Firestore Emulator** です。Google ログインは不要です。OrcaRouter / Google Maps のキーが無い状態でも、モックカタログと決定的プランナーで画面と API を通せます。LIVE は東京駅周辺を検索中心に Places から候補を取ります。LIVE 合格判定にはモックを使いません。
...
東京の発表会場住所は未提供です。開発・LIVE デモの検索中心は **東京駅周辺** です。会場そのものが確定したとは報告しません。日付は Asia/Tokyo の具体日で解決します。`DEMO_DATE` が未設定または過去日なら実行当日に読み替えます。「土曜」をモデルの今日から想像しません。
