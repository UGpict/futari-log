...
- 日付・移動手段・集合 / 解散
- 途中の行程（spot 列）
- 当時の長距離徒歩区間（区間キーと分数）と負担（最長区間・合計分）

再質問するのは、**別の長距離区間が追加された**とき、または同じ区間の徒歩が負担増になったとき。合計だけが増えても、明示的な総徒歩上限がなければ再質問しない。表示文言・item id・写真など、徒歩負担を変えない変更では失効しない（2 分までの取得ゆらぎは負担増とみなさない）。

通常の強制確認は **区間ごとの徒歩**（初期 25 分、設定変更可）。合計 45 分は表示・比較用のソフト目安であり、それだけでは `q_long_walk` を出さない。
