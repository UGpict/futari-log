...
| OrcaRouter | 設定済み | `ORCAROUTER_API_KEY`。Named Router は `orcarouter/futari-mundane` / `orcarouter/futari-hard`（詳細は `docs/specs/llm-routing.md`） |
...
| Named Router 名 | 確認済み | コンソール確認: `orcarouter/futari-mundane` / `orcarouter/futari-hard`。対応表は `docs/specs/llm-routing.md` |
